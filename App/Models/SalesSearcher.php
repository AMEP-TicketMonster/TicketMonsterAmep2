...
class SalesSearcher {
    ...
    public function findByName(string $name): ?SalesGateway {
        $stmt = $this->pdo->prepare("SELECT idSala FROM Sales WHERE nom = ?");
        $stmt->execute([$name]);
        $idSala = $stmt->fetchColumn();
        return $idSala ? new SalesGateway($idSala) : null;
    }
