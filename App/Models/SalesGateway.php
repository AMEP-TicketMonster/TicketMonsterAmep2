...
class SalesGateway {
    ...
    public function crea(string $nom, string $ciutat, int $capacitat): int {
        if ($capacitat <= 0) {
    ...
    public function modifica(string $nouNom, string $novaCiutat, int $novaCapacitat): void {
        ...
        $stmt = $this->pdo->prepare("UPDATE Sales SET nom = ?, ciutat = ?, capacitat = ? WHERE idSala = ?");
        $stmt->execute([$nouNom, $novaCiutat, $novaCapacitat, $this->idSala]);
        $this->nom = $nouNom;
